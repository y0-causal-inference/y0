...
class TestOriginalIDStar(cases.GraphTestCase):
    """Tests for the ``ID*`` algorithm."""
    ...
    def test_original_id_star_line_6(self):
        ...
        actual_summand, actual_district_events = original_id_star_line_6(input_graph, input_event)
        self.assertEqual(expected_summand, actual_summand)

        # Original ID* intervenes on all variables not in the district, but this creates problems with identifiability
        self.assertEqual(expected_district_interventions, actual_district_events)

        # The identifiability issues are resolved by intervening only on the Markov pillow.
        self.assertNotEqual(expected_markov_pillow_events, actual_district_events)
    ...
    def test_id_star_line_8(self):
        ...
        self.assertEqual({-D, +X}, get_cf_interventions(input_graph))
        query1 = {Y @ +x: +y, X: -x, D @ -d: -d}
        self.assertEqual({+x, -x, -d, +y}, get_evidence(query1))
        self.assertTrue(get_conflicts(input_graph, query1))
        query2 = {Z @ -d: -z, X: +x}
        self.assertEqual({-z, +x, -d}, get_evidence(query2))
        self.assertFalse(get_conflicts(input_graph, query2))
        graph3 = NxMixedGraph.from_edges(undirected=[(X, Y @ (-w, +x, -z))])
        event3 = {Y @ (-w, +x, -z): -y, X: -x}
        self.assertTrue(get_conflicts(graph3, event3))
        graph4 = NxMixedGraph.from_edges(directed=[(X, Y @ -x)], undirected=[(X, Y @ -x)])
        event4 = {Y @ -x: -y, X: +x}
        self.assertTrue(get_conflicts(graph4, event4))
