...
class TestChain(unittest.TestCase):
    ...
    def test_chain_expand_automatic_reordering(self):
        ...
        self.assertEqual(P(X | Y) * P(Y), chain_expand(P(X, Y)))
        self.assertEqual(P(X | (Y, Z)) * P(Y | Z) * P(Z), chain_expand(P(X, Y, Z)))
        self.assertEqual(
            P(W | (X, Y, Z)) * P(X | (Y, Z)) * P(Y | Z) * P(Z),
            chain_expand(P(W, X, Y, Z)),
        )

        # Test that conditions come along for the ride
        self.assertEqual(P(X | (Y, A)) * P(Y | A), chain_expand(P(X & Y | A)))
        self.assertEqual(
            P(X | (Y, A, B)) * P(Y | (A, B)),
            chain_expand(P(X & Y | (A, B))),
        )
