...
class TestCounterfactualAncestors(unittest.TestCase):
    """Gets counterfactual ancestors of an input graph.

    These tests are based on the examples in Example 2.1 in Counterfactual Transportability: A Formal Approach.
    """

    def test_get_counterfactual_ancestors(self) -> None:
        """Return the ancestors of a counterfactual, as a graph."""
    ...
    def make_figure_2a(self):
        ...
        graph_2a.add_directed_edge("Z", "Y")
        graph_2a.add_directed_edge("W", "Y")
        graph_2a.add_directed_edge("X", "W")
        graph_2a.add_undirected_edge("Z", "X")
        graph_2a.add_undirected_edge("W", "Y")
        print(
            self.get_counterfactual_ancestors(
                graph_2a,
            )
        )
