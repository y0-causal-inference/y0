...
def add_confound(graph, confound_pairs):
        """ add a list of pairs of nodes that share unobserved confounders"""
        
        graph_c = nx.Graph()
        graph_c.add_nodes_from(graph.nodes)
...
"""
        ...
        if graph_in is None:
            graph_temp = nx.DiGraph(graph)
        else:
            graph_temp = graph_in
    
        if p_in is None:
            p_expr = "P(" + str_list(graph_temp.nodes) + ")"
        else:
            p_expr = p_in
        ...
        # line 3
            ...
            if len(s_sets) > 1:
                ...
                for item in s_sets:
                    ...
                    str_out += id_alg(graph, graph_c, s_in, v_s_set, p_expr, graph_temp)
