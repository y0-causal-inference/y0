...
from ...dsl import Event, Expression, P, Zero
...
def get_delta_event(event: Event) -> Event:
    """Return a reduced event that doesn't have any conditionals in it."""
    raise NotImplementedError


def is_event_conditional(query: Event) -> bool:
...
def idc_star(graph: NxMixedGraph, query: Event) -> Expression:
    ...
    for counterfactual in new_delta:
        ...
        if are_d_separated(new_graph.remove_out_edges(counterfactual), counterfactual, new_event):
            counterfactual_value = counterfactual.parent()
