...
class CounterfactualVariable(Variable):
    ...
    def remove_interventions(self, variables: VariableHint) -> Variable:
        ...
        """
        _interventions = _to_interventions(_upgrade_ordering(variables))
        new_interventions = tuple(sorted(set(self.interventions) - set(_interventions)))
        if len(new_interventions) == 0:
            return Variable(name=self.name, star=self.star)
        else:
            return CounterfactualVariable(
                name=self.name,
                star=self.star,
...
class Expression(Element, ABC):
    ...
    def conditional(self, ranges: VariableHint) -> Fraction:
        ...
        return Fraction(self, self.marginalize(set(self._iter_variables()) - set(ranges)))
