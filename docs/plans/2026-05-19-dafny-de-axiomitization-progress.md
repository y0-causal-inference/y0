...
| DA-P5-003 | P5 | Resolve the PMF outcome versus node-assignment representation mismatch before constructor-level probability semantics. | Planned | Choose a shared sample-space representation or a verified translation layer connecting `Prob.Outcome`-keyed PMFs to `Assignment = map<Node, Prob.Outcome>` in `interventional.dfy`, then use it to unblock `ProductPMF`, `Marginalize`, `TruncatePMF`, and `IntProbConcrete`. | Documentation or focused Dafny verify. | `docs(dafny): plan probability assignment alignment` |
...
| 2026-05-21 | DA-P5-003 PMF outcome versus assignment alignment | Planned | Resolve the current representation mismatch between abstract `Prob.Outcome`-keyed PMFs and node-assignment semantics in `interventional.dfy`, choosing either a shared sample-space representation or a verified translation layer before constructor-level probability work continues. | Begin with a small design note and a concrete boundary inventory for `ProductPMF`, `Marginalize`, `TruncatePMF`, and `IntProbConcrete`. |
