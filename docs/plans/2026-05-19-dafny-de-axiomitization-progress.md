...
| DA-P5-003 | P5 | Resolve the PMF outcome versus node-assignment representation mismatch before constructor-level probability semantics. | Planned | Design note recorded in `docs/plans/2026-05-21-dafny-probability-assignment-alignment.md`, the first translation-layer prototype landed in `DA-P5-004`, `DA-P5-005` added reusable assignment-event algebra plus sharper `TruncatePMF` consequences, `DA-P5-006` made `Marginalize` graph-aware, and `DA-P5-007` added a graph-aware `ProductPMF` wrapper with explicit factor scopes. The remaining constructor-level work is to concretize the remaining PMF constructors and retire the new product-grounding axiom at the translation boundary. | Documentation or focused Dafny verify. | `docs(dafny): plan probability assignment alignment` |
...
| 2026-05-22 | Working tree | 86 | 19 | 67 | +1 declaration axiom | `DA-P5-007` added a graph-aware `ProductPMF` wrapper and Line 4 scope threading; only `ProductPMF_Grounded` remains axiomatic, while the wrapper itself and `ProductPMF_IsDistribution` are non-axiomatic. |
...
| DA-P5-007 | `proof(dafny): add graph-aware product pmf boundary` | Add the interventional `ProductPMF` wrapper with explicit factor scopes, expose the Line 4 scope-alignment facts needed by `IDImpl`, add `ProductPMF_Grounded`, and refresh the tracker and ledger. | Full-stack verify. | Ready |
