...
| DA-P5-001 | `docs(dafny): decide probability finite-sum proof path` | Add the probability finite-sum decision note and update the progress tracker with the chosen `ProbEvent` direction. | Markdown diff check. | Ready |
| DA-P5-002 | `proof(dafny): define finite-sum event probability` | Add the finite-support summation helper and concrete `ProbEvent`, then prove `ComplementRule`, `InclusionExclusion`, and `TotalProbability`. | Focused `probability.dfy` verify plus full-stack verify. | Ready |
