...
| `CComponentCompiled_Correct` | 658 | Infrastructure equivalence | P3 (In progress) | Forward inclusion (`CComponentCompiled <= CComponent`) is now proved via `BidirectedNeighbors_HasBidirected`, `BidirectedBFSLoop_SubsetCComponent`, and `CComponentCompiled_Subset_CComponent`; simple-path substrate is proved via `NoDupPath`, `NoDupPath_BoundedBySet`, and `IsBiPath_SimplifyNoDup`; remaining work is reverse inclusion (`CComponent <= CComponentCompiled`) via a stronger BFS path-following invariant, then remove `{:axiom}`. |
