...
| `Axiom_Normalization` | 109 | Derivable wrapper | Done | No longer axiomatic. `SumsToOne(p)` is now the concrete predicate `ProbEvent(p, p.Keys) == 1.0`, so `Axiom_Normalization` is now a direct proof body instead of a trusted probability boundary. |
...
| `TruncateSupportAssignments` | 868 | Abstract interface | P5 | Finite assignment-side support boundary for future PMF-valued truncation; kept axiomatic because the repo still lacks a concrete finite generator of full assignments for the factor-level constructor. Its contracts now include the smallest completeness boundary the current constructor proof needs: every positive-mass truncated full assignment must appear in the finite support sequence. |
| `SumTruncatedAssignmentMasses_Normalized` | 1729 | Derivable wrapper | P5 | New exact normalization boundary for the order-parameterized truncation constructor. The surrounding proof is now concrete on both sides: `Prob.SumsToOne` is concrete and `TruncatePMFOnOrder_IsDistribution` is proved. What remains axiomatic is the recursive theorem that the chosen truncated-support sequence sums to `1.0` for arbitrary topological orders. |
| `TruncatePMF` | 1071 | Abstract interface | P5 | The support-filtering/renormalization probe was rejected as the public semantics. There is now a concrete order-parameterized helper `TruncatePMFOnOrder` over `EncodedTruncateSupport(G, p, X, xVals)`, but the public constructor remains abstract because an order-free wrapper still needs an internal order or full-node-enumeration witness, and the current bridge only canonically re-encodes full assignments in one direction. |
| `TruncatePMF_IsDistribution` | 1083 | Derivable wrapper | P5 | The order-parameterized constructor now already has a proved `Prob.IsDistribution` theorem. The remaining blocker is the public wrapper boundary: `TruncatePMF` itself is still abstract because the repo has not yet introduced an order-free topological witness / representation theorem for the public constructor. |
| `TruncatePMF_Empty` | 1092 | Derivable wrapper | P5 | The first empty-intervention bridge now exists at constructor level: on encoded support, `TruncatePMFOnOrder(G, p, {}, map[], ord)` masses reduce to `ConditionalFactorProduct`. The remaining public equality to `p` needs both an internal order witness for the public wrapper and a stronger representation theorem than the current left-inverse-only bridge, in addition to the eventual Markov-factorization grounding. |
| `TruncatePMF_Markov` | 1098 | Deep theorem | P5 | Expected to align with `RemoveIncoming(G, X)` by construction once the factor-level `TruncatePMFOnOrder` semantics are tied to a public wrapper and the remaining factorization/representation theorems are in place. |
...
| `ProductPMF_Grounded` | 1341 | Derivable wrapper | P5 | New graph-aware grounding point for factor products: the interventional wrapper itself is concrete over `Prob.ProductPMF`, but the assignment-level factorization law still remains axiomatic until the factor product semantics are made concrete. |
| `Marginalize` | 1365 | Abstract interface | P5 | Still abstract, but now graph-aware and constrained to preserve `AssignmentProb` for partial assignments whose keys avoid the eliminated set. Remaining work is the concrete summation construction. |
| `Marginalize_IsDistribution` | 1371 | Derivable wrapper | P5 | Prove once `Marginalize` is concrete. |
