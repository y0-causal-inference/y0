...
| `TruncateSupportAssignments` | 749 | Abstract interface | P5 | Finite assignment-side support boundary for future PMF-valued truncation; kept axiomatic because the repo still lacks a concrete finite generator of full assignments for the factor-level constructor. Its contracts now feed the first concrete order-parameterized constructor helper through `EncodedTruncateAssignment`, while still exposing the intended shape of future truncated support: full assignments over `Nodes(G)`, each extending `xVals`, with distinct encodings under `AssignmentToOutcome`. |
| `TruncatePMF` | 900 | Abstract interface | P5 | The support-filtering/renormalization probe was rejected as the public semantics. There is now a concrete order-parameterized helper `TruncatePMFOnOrder` over `EncodedTruncateSupport(G, p, X, xVals)`, but the public constructor remains abstract because the signature still lacks a canonical topological-order witness and the current bridge only canonically re-encodes full assignments in one direction. |
| `TruncatePMF_IsDistribution` | 912 | Derivable wrapper | P5 | The proof path is now through the concrete `TruncatePMFOnOrder` helper rather than through observational renormalization. Remaining work is conditional-factor normalization over the encoded-support constructor and then a public wrapper boundary. |
| `TruncatePMF_Empty` | 921 | Derivable wrapper | P5 | The first empty-intervention bridge now exists at constructor level: on encoded support, `TruncatePMFOnOrder(G, p, {}, map[], ord)` masses reduce to `ConditionalFactorProduct`. The remaining public equality to `p` still needs Markov-factorization grounding plus a stronger representation theorem than the current left-inverse-only bridge. |
| `TruncatePMF_Markov` | 927 | Deep theorem | P5 | Expected to align with `RemoveIncoming(G, X)` by construction once the factor-level `TruncatePMFOnOrder` semantics are tied to a public wrapper and the remaining factorization/representation theorems are in place. |
| `IntProbConcrete` | 1055 | Abstract interface | Done | Replaced in the 2026-05-21 working tree with a concrete assignment-conditional probability on the truncated PMF. |
| `IntProb_Grounded` | 1085 | Contract gap | Done | Replaced in the 2026-05-21 working tree with an explicit equality contract relating `IntProbConcrete` to `AssignmentCondProb` on truncated-PMF assignment events. |
| `GlobalMarkov_From_Factorization` | 1122 | Contract gap | P6 | Intended deep theorem, but currently lacks a formal postcondition. |
| `ProductPMF_Grounded` | 1170 | Derivable wrapper | P5 | New graph-aware grounding point for factor products: the interventional wrapper itself is concrete over `Prob.ProductPMF`, but the assignment-level factorization law still remains axiomatic until the factor product semantics are made concrete. |
| `Marginalize` | 1194 | Abstract interface | P5 | Still abstract, but now graph-aware and constrained to preserve `AssignmentProb` for partial assignments whose keys avoid the eliminated set. Remaining work is the concrete summation construction. |
| `Marginalize_IsDistribution` | 1200 | Derivable wrapper | P5 | Prove once `Marginalize` is concrete. |
