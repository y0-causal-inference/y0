...
| `TruncatePMF_IsDistribution` | 1083 | Derivable wrapper | P5 | The concrete constructor path now has `AllNonNeg`, positive-mass support completeness, and a local conditional-PMF normalization/locality boundary. The next blocker is no longer local truncation structure; it is the abstract `Prob.SumsToOne` boundary in `probability.dfy`, which still prevents a concrete normalization theorem from implying `Prob.IsDistribution`. |
