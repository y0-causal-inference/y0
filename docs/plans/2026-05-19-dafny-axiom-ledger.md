...
| `ProductPMF_Grounded` | 1341 | Derivable wrapper | P5 | New graph-aware grounding point for factor products: the interventional wrapper itself is concrete over `Prob.ProductPMF`, but the assignment-level factorization law still remains axiomatic until the factor product semantics are made concrete. This is now one of the last non-do-calculus P5 semantic boundaries. |
